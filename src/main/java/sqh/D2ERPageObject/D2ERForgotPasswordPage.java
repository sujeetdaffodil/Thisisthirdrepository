...
public class D2ERForgotPasswordPage {
	WebDriver driver;
...
//	@FindBy(xpath="//a[text()= 'reset link']/preceding::text()[2]")
//	private WebElement tempVerificationCode;
	...
	public D2ERForgotPasswordPage(WebDriver driver) {
		PageFactory.initElements(driver, this);
		this.driver=driver;
		this.wait = new WebDriverWait(driver, Duration.ofSeconds(60));
	}
	
	//Function for Entering Email on Forgot Password Page
	...
	// Function for Resetting Password using Tempmamil
	public void PasswordResetEmailUsingTempmail(String forgotPwdUsername)
	{
		...
		Boolean IsEnteredTempmail = driver.findElements(By.xpath("//p[contains(.,'Disposable address: ')]")).size()>0;
	...
	// Function for Resetting Password using Mailinator
	...
	// Function for extracting Verification Code from Email Template
	...
	// Function to Set new Password to the email for which forget Password flow is triggered
	public WebElement SetNewPassword()
	{    Rnum= RandomStringGeneratorOperations.randomString(2);

	driver.findElement(By.cssSelector("Body")).sendKeys(Keys.CONTROL,Keys.RETURN);
	driver.get(resetLink.getAttribute("href"));
	...
	public String ForgetPasswordPageTitle() {
		return WebDriverOperations.GetTitle(driver);
	}
